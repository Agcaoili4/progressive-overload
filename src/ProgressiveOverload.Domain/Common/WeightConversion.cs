...
public static class WeightConversion
{
    ...
    */
    ...
    /*
        Metric just rounds to a sane display precision. Imperial snaps to the nearest 0.5 lb:
        a lifter who enters 225 lb has it stored as 102.06 kg, and converting straight back
        gives 224.9868 lb. Showing "224.99 lb" for a lift they know was 225 reads as a bug.
        Lifters think in plate increments, so snapping to the nearest half pound restores the
        number they actually lifted while still representing micro-plate loads like 227.5 lb.

        Both branches round AwayFromZero because the .NET default (banker's rounding) would
        make a value like 0.25 round to 0.0, which would surprise everyone.
    */
